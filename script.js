...
const generateGame = () => {
    ...
    const flipCard = card => {
        ...
        if ( !document.querySelectorAll(".card:not(.flipped)").length ) {
            setTimeout(() => {
                ...
                selectors.win.innerHTML = `<span class='win-text'>You won! <br> with <span class='highlight'>${state.totalFlips}</span> moves <br> under <span class="highlight">${state.totalTime}</span>sec</span>`
    ...
    const attachEventListener = () => {
        document.addEventListener('click', event=> {
            ...
            !eventParent.className.includes('disabled')){
                startGame()
            }
        })
    }
